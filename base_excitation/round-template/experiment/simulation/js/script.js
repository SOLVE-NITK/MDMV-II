        ...
        var mass=165;                                  //Mass of the system
        var base_mass=10;                             //Mass of base
        ...
        var nfreq=rot_freq/wn;
        var phi=Math.atan(2*dampingratio*nfreq/1-(nfreq*nfreq));
        var cc=2*mass*wn;
        ...
        var stop_time=0;
        var offsetg=0;                                 //For graph animation
        var offsetf=0;                                 //For graph animation


        //Function to calculate the displacement
        function displacement(t){
         let value=base_displacement(t)*Math.sqrt((1+Math.pow(2*dampingratio*nfreq,2))/(Math.pow(1-(nfreq*nfreq),2)+Math.pow(2*dampingratio*nfreq,2)));
         return 0.03*value;
        }

        function base_displacement(t){
            let value=40*((((base_mass*eccentricity)/mass)*nfreq*nfreq)/Math.sqrt(Math.pow((1-nfreq*nfreq),2)+Math.pow(2*dampingratio*nfreq,2))*Math.sin(rot_freq*t+phi));
            return value;
        }
       ...
       //start of simulation here; starts the timer with increments of 0.01 seconds
        ...
        function varchange()
        {
          ...
          $('#frequencyspinner').spinner({ max : 500, min : 0, step : 0.1 });
          $( "#frequencyspinner" ).on( "change", function() {  varchange()} );
            ...
            $('#massslider').slider({ max : 165, min : 2, step : 0.5 });
            $('#massspinner').spinner({ max : 165, min : 2, step : 0.5 });
        ...
        function varupdate(){
            ...
            cc=2*mass*wn;
            base_mass=$('#forcespinner').spinner("value");
            rot_freq=$('#frequencyspinner').spinner("value");
            nfreq=rot_freq/wn;
            phi=Math.atan(2*dampingratio*nfreq/1-(nfreq*nfreq));
        ...
        function draw() {
            ...
            let spring = {
                initsprx: [150, 150, 170, 130, 170, 130, 170, 130, 170, 130, 170, 130, 170, 130, 150, 150],
            ...
            ctx.beginPath();
            ctx.fillRect(30, 360+offset, 240, 70);
            ctx.beginPath();
            ctx.fillStyle='#ffffff';
            ctx.arc(150,395+offset,35,2*Math.PI,false);
            ctx.fill();
            ctx.beginPath();
            ctx.fillStyle='ff0000';
            ctx.arc(150+30*Math.sin(rot_freq*time),offset+395-30*Math.cos(rot_freq*time),5,2*Math.PI,false);
            ctx.fill();
            for(let r=0;r<=15;r++){
                spring.spry[r]+=offset;
            }
            spring.draw();
            ctx.fillStyle = "#663300";//brown color for mass
            ...
            ctx.lineTo(230, 220+offset);
            ctx.moveTo(192, 330+offset);
            ctx.lineTo(192, 220+offset);
            ctx.stroke();
            ctx.fillStyle = "#bae8fe";//blue for damper
            ctx.fillRect(194, 220+offset, 34, 108);
            ctx.beginPath();
            ctx.strokeStyle = "#670090";//damper
            ...
            ctx.save();
            ctx.translate(320,395);
            ctx.rotate(-Math.PI/2);
            ctx.font="18px Comic Sans MS";
            let st="2";
            ctx.fillText("Base Displacement",-50,-10);
            ctx.restore();
            ctx.beginPath();
            ctx.moveTo(320, 370-base_displacement(0.01*offsetf));
            ...
            if (!simstatus)
            {
                ...
                while(i<xf){
                    ctx.lineTo(320+i, 370 - base_displacement(0.01*(i+offsetf)));
                    ctx.stroke();
                    i+=0.7;
                }
                xvalf=xf;
            ...
            if(simstatus){
                let l=0;
                while(l<xvalf){
                    ctx.lineTo(320+l, 370 - base_displacement(0.01*(l+offsetf)));
                    ctx.stroke();
                    l+=0.7;
                }
            }
            ...
            ctx.fillStyle="black";
            ctx.font="18px Comic Sans MS";
            ctx.fillText('t',540,140);
            ctx.save();
            ctx.translate(320,115);
            ctx.rotate(-Math.PI/2);
            ctx.fillText("Displacement",-90,-10);
            ctx.restore();
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(320, 140-1.5*displacement(0.01*offsetg));
            ...
            if (!simstatus)
            {
                ...
                while(i<x){
                    ctx.lineTo(320+i, 140 - 1.5*displacement(0.01*(i+offsetg)));
                    ctx.stroke();
                    i+=0.7;
                }
                xval=x;
            ...
            if(simstatus){
                let l=0;
                while(l<xval){
                    ctx.lineTo(320+l, 140 - 1.5*displacement(0.01*(l+offsetg)));
                    ctx.stroke();
                    l+=0.7;
                }
            }
        ...
        function plotgraph(){
            ...
            if(xg>290) {
                let ny = 230 - 0.25*magnifac(nfreq);
                if (ny <= 0)
                    ny = 5;
                ctxgraph.arc(70 + 100 * nfreq, ny, 3, 0, 2 * Math.PI, false);
                ctxgraph.stroke();
            }
            ctx.beginPath();
            ctx.lineWidth=1;
            ...
            ctxgraph.lineWidth = 1;
            if(xg<(100+nfreq)) {
                let ny2 = 400 - phaseangle(nfreq);
                ctxgraph.arc(70 + 100 * nfreq, ny2, 3, 0, 2 * Math.PI, false);
                ctxgraph.stroke();
            }
        }
